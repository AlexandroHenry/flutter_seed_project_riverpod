...
import 'verify/verify_code_screen.dart';
    ...
    ref.listen<ForgotPasswordState>(forgotPasswordProvider, (prev, state) {
      if ((prev?.sent != state.sent) && state.sent) {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => _VerifyCodeScreenEntry(email: _emailController.text.trim()),
          ),
        );
      }
    });
...
/// Inline small entry widget to avoid routing churn; real screen is defined below.
class _VerifyCodeScreenEntry extends StatelessWidget {
  const _VerifyCodeScreenEntry({required this.email});
  final String email;
